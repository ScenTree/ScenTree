...
// args
const the_third_arg = process.argv[3 -1]; // the path of the file usually called descriptors__descripteurs.html
...
if (! fs.existsSync(the_third_arg)) {
    console.log("The file '" + the_third_arg + "' does not exist -> STOP! Please provide a 'descriptors__descripteurs.html' file in argument (1st place)" );
    return;
} else {
...
function empty_a_html_element(the_html_element) {
...
function add_one_object_to_a_dom(the_dom, the_object) {
    ...
    if (is_a_famille_principale(the_object)) {	// if the object is famille principale
    ...
    if (is_a_famille_principale(the_object)) {
...
async function complete_the_html_file(the_json_file) {
	...
	console.log("The file " + the_html_file + " was saved !");
    });
};
